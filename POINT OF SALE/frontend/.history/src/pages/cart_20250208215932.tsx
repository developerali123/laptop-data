...
const Cart = ({ cartItems, deleteCartItem, subtotal, tax, total, discount, appliedCoupons }) => {
  ...
  const [isCouponModalOpen, setIsCouponModalOpen] = useState(false); // ✅ Added state for coupon modal
  const [isDiscountModalOpen, setIsDiscountModalOpen] = useState(false); // ✅ Added state for discount modal

  if (isPaymentPageVisible) {
    return <PaymentPage total={total} goBack={() => setIsPaymentPageVisible(false)} />;
  }

  return (
    <div className="w-[40%] pl-4 max-h-[calc(100vh-100px)] overflow-y-auto">
      <h2 className="text-xl font-semibold flex items-center gap-2">
        <FaShoppingCart aria-label="Shopping Cart" /> Cart Items
      </h2>
      ...
      <aside className="fixed bottom-0 left-0 right-0 bg-white shadow-lg rounded-t-lg p-4 w-full max-w-md ml-auto">
        ...
        <div className="grid grid-cols-3 gap-2 mt-4">
          <button className="flex items-center justify-center gap-2 p-2 bg-gray-300 rounded-lg" onClick={() => setIsCouponModalOpen(!isCouponModalOpen)}>
            <FaTag aria-label="Coupon" /> Coupon
          </button>
          <button className="flex items-center justify-center gap-2 p-2 bg-gray-300 rounded-lg" onClick={() => setIsDiscountModalOpen(!isDiscountModalOpen)}>
            <FaPercentage aria-label="Discount" /> Discount
          </button>
          <button className="flex items-center justify-center gap-2 p-2 bg-gray-300 rounded-lg">
            <FaPause aria-label="Hold Order" /> Hold Order
          </button>
        </div>

        <button onClick={() => setIsPaymentPageVisible(true)} className="mt-4 w-full p-3 text-white bg-blue-600 rounded-lg text-lg font-semibold flex justify-between">
