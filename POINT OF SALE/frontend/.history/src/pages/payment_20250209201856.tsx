      ...
      <div className="flex-1 bor p-8">
        ...
        <div className="grid grid-cols-4 hover:border-blue-700 gap-2 mt-4 text-center">
          {[1, 2, 3, "clear", 4, 5, 6, "←", 7, 8, 9, "Pay", 0, ".", "00", "Cancel"].map((key) => (
            <button key={key} className={`p-3 hover:border-blue-700 ${key === "Pay" ? "bg-blue-600 hover:border-blue-700 text-white" : "bg-gray-200 hover:border-blue-700"} rounded`} onClick={() => handleKeypadClick(key)}>
              {key}
            </button>
          ))}
        </div>
