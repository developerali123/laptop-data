...
const Cart = ({
  ...
  discount,
  appliedCoupons,
  setIsCouponModalOpen,  // <-- Ensure these props are passed
  setIsDiscountModalOpen // <-- Ensure these props are passed
}) => {
  return (
      ...
      <aside className="fixed bottom-0 left-0 right-0 bg-white shadow-lg rounded-t-lg p-4 w-full max-w-md ml-auto">
        ...
        <div className="grid grid-cols-3 gap-2 mt-4">
          <button
            ...
            onClick={() => setIsCouponModalOpen(prev => !prev)} // Corrected function call
          >
            <FaTag /> Coupon
          </button>
          <button
            className="flex items-center justify-center gap-2 p-2 bg-gray-300 rounded-lg"
            onClick={() => setIsDiscountModalOpen(prev => !prev)} // Corrected function call
          >
